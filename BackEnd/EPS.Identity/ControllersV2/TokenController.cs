...
using EPS.Libary.Models;
using EPS.Libary.RabbitManager;
using EPS.Libary.RabbitManager.Dtos;
...
namespace EPS.Identity.Controllers
{
    ...
    public class TokenController : BaseController
    {
        ...
        private async Task<IActionResult> SendNotificationToAdmin(string userName)
        {
            ...
            if (user != null)
            {
                var userPassword = await _context.PasswordResetRequest.Where(x => x.UserId ==  user.Id && x.status == (int)PasswordResetRequestEnum.REQUEST && !x.DeletedDate.HasValue).FirstOrDefaultAsync();
        ...
        private async Task<PasswordResetRequest?> CreateOrUpdatePassword(int userId)
        {
            try
            {
                var data = await _context.PasswordResetRequest.Where(s =>s.UserId == userId && !s.DeletedDate.HasValue).FirstOrDefaultAsync();
                if(data != null)
                {
                ...
                else
                {
                    ...
                    await _context.PasswordResetRequest.AddAsync(input);
                    await _context.SaveChangesAsync();
