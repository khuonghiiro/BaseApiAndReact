...
export const UploadFile = ({
  ...
  function configFile(files: any) {
    ...
    // Thực hiện các thao tác cần thiết sau đó

    // Truyền vào state chỉ những file hợp lệ
    setInnerValue([...validFiles]);
    setfileAttach([...validFiles]);
  ...
  const gettime = async () => {
    debugger;
    let res = null;
    res = await api.get("fileupload/gettime");
    return res;
  };

  // const onDownload = async (id: any, time: any) => {
  //   debugger;
  //   time = await userServices.gettime();
  //   // let a = document.createElement("a");
  //   // a.href = ApiUrl + url + "/" + res;
  //   // a.target = "_blank";
  //   // a.click();
  //   let res = userServices.onDownload(id, time);
  //   console.log("res", res);
  // };
