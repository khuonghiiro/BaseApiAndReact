...
import GroupTourGuideForm from "./_components/grouptourguide-form";
...
export default function Page() {
  ...
  const { data, isLoading, mutate } = groupTourGuideServices.GetList(meta);
    ...
    <>
      <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
        ...
        >
          ...
          />
          <GridView.Table.Column style={{}} title="Tên hướng dẫn" sortKey="name" body={({ item }) => (<span>{item.name}</span>)} />
          <GridView.Table.Column style={{}} title="Kích hoạt" sortKey="isActive" body={({ item }) => (<span>{item.isActive}</span>)} />
          <GridView.Table.Column
            style={{ width: "10%" }}
            ...
            body={({ item }) => (
              <div className="flex flex-row">
                ...
                {permisson.per_Delete && <AiFillDelete
                  ...
                  title="Xóa"
                  onClick={() => delAction(item, groupTourGuideServices, data, setMeta, meta, mutate)}
                />
                }
              </div>
            )}
      ...
      {state.show && <GroupTourGuideForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />}
      <ConfirmationDialog />
    </>
  );
}
