...
export default function Page() {
  ...
  const { data, isLoading, mutate } = testServices.GetList(meta);
  ...
  useEffect(() => {
    setPermisson(getPermisson("test"));
  }, []);
  return (
    <>
            ...
            body={({ item }) => (
              <div className="flex flex-row">
                ...
                {permisson.per_Delete && <AiFillDelete
                  ...
                  title="Xóa"
                  onClick={() => delAction(item, testServices, data, setMeta, meta, mutate)}
                />
                }
              </div>
            )}
      ...
      {state.show && <TestForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />}
      <ConfirmationDialog />
    </>
  );
}
