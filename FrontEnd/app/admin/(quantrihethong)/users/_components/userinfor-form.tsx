...
export default React.memo(function UserInforForm({
  show,
  action,
  id,
  onClose,
  ...
  const schema = object({
    ...
    email: string()
      ...
      .min(3, "Bạn nhập tối thiểu 3 ký tự")
      .max(30, "Bạn nhập tối đa 30 ký tự")
      .test("Unique Email", "Email đã tồn tại", function (value) {
        if (!value) return true;
        return new Promise(async (resolve, reject) => {
          let result = await userServices.checkTrung(value, this.parent.id);
          if (result.data.length > 0) resolve(false);
          else resolve(true);
        });
      }),
    password: string()
