...
export default function Page() {
  ...
  const { data, isLoading, mutate } = documentServices.GetList(meta);
  ...
  const handleChange = (res: any) => {
   ...
   useEffect(() => {
    setPermisson(getPermisson("document"));
  }, []);
  return (
    <>
      <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
        ...
        >
          ...
          />
          <GridView.Table.Column style={{}}title="Tên tài liệu"sortKey="title"body={({ item }) => (<span>{item.title}</span>)} />
<GridView.Table.Column style={{}}title="Nội dung"sortKey="content"body={({ item }) => (<span>{item.content}</span>)} />
          <GridView.Table.Column
            style={{ width: "10%" }}
            ...
            body={({ item }) => (
                <div className="flex flex-row">
                    ...
                    {permisson.per_Delete && <AiFillDelete
                      ...
                      title="Xóa"
                      onClick={() => delAction(item,documentServices,data,setMeta,meta,mutate)}
                    />
                }
              </div>             
            )}
      ...
      {state.show&&<DocumentForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />}
      <ConfirmationDialog />
    </>
  );
}
