...
export default function Page() {
  ...
  const { data, isLoading, mutate } = projectServices.GetList(meta);
  ...
  useEffect(() => {
    setPermisson(getPermisson("project"));
  }, []);
  return (
    <>
      <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
        ...
        >
          ...
          />
          {/* <GridView.Table.Column style={{}} title="ID của người sở hữu dự án " sortKey="ownerId" body={({ item }) => (<span>{item.ownerId}</span>)} /> */}
          <GridView.Table.Column style={{}} title="Mã dự án" sortKey="code" body={({ item }) => (<span>{item.code}</span>)} />
          <GridView.Table.Column style={{}} title="Tên dự án" sortKey="name" body={({ item }) => (<span>{item.name}</span>)} />
          <GridView.Table.Column style={{}} title="Ngày bắt đầu dự án" sortKey="startDate" body={({ item }) => (<span>{formatDateTime(item.startDate)}</span>)} />
          <GridView.Table.Column style={{}} title="Ngày kết thúc dự án" sortKey="endDate" body={({ item }) => (<span>{formatDateTime(item.endDate)}</span>)} />
          <GridView.Table.Column
            style={{ width: "10%" }}
            ...
            body={({ item }) => (
              <div className="flex flex-row">
                ...
                {permisson.per_Delete && <AiFillDelete
                  ...
                  title="Xóa"
                  onClick={() => delAction(item, projectServices, data, setMeta, meta, mutate)}
                />
                }
              </div>
            )}
      ...
      {state.show && <ProjectForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />}
      <ConfirmationDialog />
    </>
  );
}
