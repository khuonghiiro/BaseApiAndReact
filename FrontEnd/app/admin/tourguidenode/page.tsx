...
import TourGuideNodeForm from "./_components/tourguidenode-form";
...
export default function Page() {
  ...
  const { data, isLoading, mutate } = tourGuideNodeServices.GetList(meta);
    ...
    <>
      <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
        ...
        >
          ...
          />
          <GridView.Table.Column style={{}}title="Tên class hoặc id của thẻ html"sortKey="classOrId"body={({ item }) => (<span>{item.classOrId}</span>)} />
<GridView.Table.Column style={{}}title="Tiêu đề"sortKey="title"body={({ item }) => (<span>{item.title}</span>)} />
<GridView.Table.Column style={{}}title="Bước thực hiện"sortKey="stepIndex"body={({ item }) => (<span>{item.stepIndex}</span>)} />
          <GridView.Table.Column
            style={{ width: "10%" }}
            ...
            body={({ item }) => (
                <div className="flex flex-row">
                    ...
                    {permisson.per_Delete && <AiFillDelete
                      ...
                      title="Xóa"
                      onClick={() => delAction(item,tourGuideNodeServices,data,setMeta,meta,mutate)}
                    />
                }
              </div>             
            )}
      ...
      {/* {state.show&&<TourGuideNodeForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />} */}
      <ConfirmationDialog />
    </>
  );
}
